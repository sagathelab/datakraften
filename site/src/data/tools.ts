...
export const tools: Record<string, ToolDef> = {
  dk: {
    ...
    sections: [
      ...
      {
        title: 'dk profile',
        body: 'Manage Datakraften profiles -- predefined sets of tools for different developer types.\n\n$ dk profile list\n\nAvailable profiles:\n- minimal -- just system tools and git\n- default -- balanced setup for most developers\n- custom -- your own config, optionally from a remote YAML URL\n\nSwitch profile:\n\n$ dk profile use default\n$ dk apply             // Re-run apply with the new profile\n\nCustom profile with remote YAML:\n\n$ dk profile use custom\n\nIf no remote URL is configured, you will be prompted to enter one. Datakraften fetches the YAML from the URL and writes it to your local config. This lets teams share a standardized setup:\n\n$ dk profile use custom\n> Remote team YAML URL: https://example.com/team-config.yaml\n> Fetching https://example.com/team-config.yaml... ✓\n> Config updated\n\nYou can also set the URL during dk init by selecting the custom profile:\n\n$ dk init\n? Select a profile: custom\n? Remote team YAML URL (optional): https://example.com/team-config.yaml\n\nThe remote YAML uses the same format as local config, so you can define packages, runtimes, editors, and shell settings centrally for your entire team.',
      },
      {
        title: 'dk update',
        id: 'update',
