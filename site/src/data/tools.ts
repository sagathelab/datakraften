...
export const tools: Record<string, ToolDef> = {
  dk: {
    ...
    sections: [
      ...
      {
        title: 'dk profile',
        body: 'Manage Datakraften profiles -- predefined sets of tools for different developer types.\n\n$ dk profile list\n\nAvailable profiles:\n- minimal — just system tools and git\n- default — balanced setup for most developers\n- custom — local-only, edit everything yourself\n- team — shared setup from a remote YAML URL\n\nSwitch profile:\n\n$ dk profile use default\n$ dk apply\n\nNOTE: Re-run apply with the new profile\n\nTeam profile:\n\nThe team profile is designed for teams. Instead of a built-in preset, it fetches your entire toolset from a remote YAML URL. The remote file defines everything — ~system_packages~, ~brew_packages~, ~runtimes~, ~shell~, ~editors~, ~ai_tools~, and ~ai_apps~. The local config is a thin pointer — just ~profile: team~ and ~team.url~.\n\nOn `dk init` (or `dk profile use team`), you are prompted for the URL after selecting the team profile. Datakraften fetches the remote YAML to validate it, then writes only the URL to your local config. On every `dk apply`, it re-fetches the remote YAML fresh — so the team config is always the single source of truth.\n\n$ dk profile use team\n> Team config URL: https://example.com/team-config.yaml\n> Fetching remote config from https://example.com/team-config.yaml...\n> ✓ Remote config validated\n\n$ dk apply\n\nNOTE: The remote URL is required — there is no fallback to a local config. If the remote YAML is invalid or unreachable, `dk apply` aborts with an error.',
      },
      {
        title: 'dk update',
        id: 'update',
  ...
  config: {
    ...
    sections: [
      {
        title: 'Overview',
        body: 'Datakraften uses a single YAML file at ~/.config/datakraften/config.yaml to define your development environment. For most profiles, every setting lives in this one file. For the team profile, it is a thin pointer — just ~profile: team~ and ~team.url~ — and the actual tool settings are fetched fresh from the remote URL on each `dk apply`.\n\nThe config is split into top-level sections, each controlling a different aspect of your workstation. Here is the full structure:',
      },
      {
        title: 'profile',
        body: 'The profile field selects which toolset to apply. Four built-in profiles:\n\n- minimal — just system essentials and git\n- default — balanced setup for most developers (runtimes, shell, editors, AI tools)\n- custom — local-only, edit everything yourself\n- team — shared setup from a remote YAML URL (single source of truth)\n\n$ dk profile list\n\nWhen team is active, a ~team.url~ field must be set pointing to a YAML file hosted anywhere accessible via HTTPS. The remote YAML defines every tool setting — ~system_packages~, ~brew_packages~, ~runtimes~, ~shell~, ~editors~, ~ai_tools~, and ~ai_apps~. The local config stores only ~profile: team~ and ~team.url~; it is never a full config.\n\nOn `dk init` (after selecting the team profile) or `dk profile use team`, you are prompted for the URL. Datakraften fetches and validates the remote YAML immediately. On every `dk apply`, it re-fetches the remote YAML fresh — the remote file is always the single source of truth. If the URL is unreachable or the YAML is invalid, `dk apply` aborts with an error.\n\n$ dk profile use team\n> Team config URL: https://example.com/team-config.yaml\n> Fetching remote config from https://example.com/team-config.yaml...\n> ✓ Remote config validated\n\n$ dk apply\n> Fetching remote config from https://example.com/team-config.yaml...\n> ✓ Remote config loaded\n\nNOTE: Because the remote YAML is fetched fresh each time, updates to the shared config take effect immediately on the next `dk apply` — no need to re-run `dk init`.',
      },
      {
