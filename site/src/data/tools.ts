...
export const tools: Record<string, ToolDef> = {
  dk: {
    ...
    sections: [
      ...
      {
        title: 'dk profile',
        body: 'Manage Datakraften profiles -- predefined sets of tools for different developer types.\n\n$ dk profile list\n\nAvailable profiles:\n- minimal — just system tools and git\n- default — balanced setup for most developers\n- custom — local-only, edit everything yourself\n- team — shared setup from a remote YAML URL\n\nSwitch profile:\n\n$ dk profile use default\n$ dk apply\n\nNOTE: Re-run apply with the new profile\n\nTeam profile:\n\nThe team profile is designed for teams. Instead of a built-in preset, it fetches your entire toolset from a remote YAML URL. The remote file defines everything — ~system_packages~, ~brew_packages~, ~runtimes~, ~shell~, ~editors~, ~ai_tools~, and ~ai_apps~. The local config only stores:\n- ~profile: team~\n- ~team.url~\n\nWhen you run `dk init`, Datakraften fetches the remote YAML and writes it to your local config. On `dk apply`, it reads from local state — every developer with the same URL gets the exact same environment.\n\nSet the URL during initial setup:\n\n$ dk init\n\nOr switch later:\n\n$ dk profile use team\n> Remote team YAML URL: https://example.com/team-config.yaml\n> Fetching... ✓\n\nShare the URL and every developer on your team gets the same stack.',
      },
      {
        title: 'dk update',
        id: 'update',
  ...
  config: {
    ...
    sections: [
      ...
      {
        title: 'profile',
        body: 'The profile field selects which toolset to apply. Four built-in profiles:\n\n- minimal — just system essentials and git\n- default — balanced setup for most developers (runtimes, shell, editors, AI tools)\n- custom — local-only, edit everything yourself\n- team — shared setup from a remote YAML URL (single source of truth)\n\n$ dk profile list\n\nWhen team is active, a ~team.url~ field is set pointing to a YAML file hosted anywhere accessible via HTTPS. The remote YAML replaces all local tool settings entirely — ~system_packages~, ~brew_packages~, ~runtimes~, ~shell~, ~editors~, ~ai_tools~, and ~ai_apps~ are all defined in that remote file. The local config only stores which profile to use and where to fetch it.\n\nSet the URL during init:\n$ dk init\n\nOr switch later:\n$ dk profile use team\n> Remote team YAML URL: https://example.com/team-config.yaml\n> Fetching... ✓\n\nDatakraften fetches the YAML fresh and writes it to your local config on `dk init`. On `dk apply`, it reads the config from local state — no re-fetch. Share the URL with your team and everyone gets identical environments.',
      },
      {
