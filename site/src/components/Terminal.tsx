...
const tabs = [
  ...
  { id: 'bash', label: 'Bash (legacy)', command: 'bash <(curl -fsSL https://datakraften.no/wsl/debian.sh)' },
]
  ...
  return (
    <div className="rounded-lg overflow-hidden border border-fuchsia-500/30 bg-[#1a1a2e] max-w-2xl mx-auto">
      <div className="flex items-center gap-1.5 px-3 py-2 bg-black/30">
        ...
        <span className="ml-4 text-xs text-text-dim font-jetbrains">{activeTab === 'dk' ? 'dk-cli' : 'bash-legacy'}</span>
      </div>
      <div className="flex border-b border-fuchsia-500/20">
        {tabs.map(tab => (
          <button
            ...
            className={`px-4 py-2 text-xs font-jetbrains transition-colors ${
              activeTab === tab.id
      ...
      <div className="p-4 font-jetbrains text-sm leading-relaxed flex justify-between items-start">
        <div>
          <span className="text-prompt">$</span>
          <span className="ml-2 text-text-primary">{activeCommand}</span>
          <span className="inline-block w-2 h-5 bg-magenta ml-1 animate-cursor-blink" />
        </div>
        <button
          onClick={copyCommand}
      ...
      <div className="px-4 pb-2 text-xs text-text-dim">
        {activeTab === 'dk' ? 'Install the Datakraften CLI' : 'Legacy bash installer for WSL Debian/Ubuntu'}
      </div>

      {showToast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-[#1a1a2e] border border-fuchsia-500/30 px-4 py-2 rounded text-sm text-text-primary font-jetbrains z-50 animate-blink">
          ✓ Copied to clipboard
        </div>
      )}
    </div>
